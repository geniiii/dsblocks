...
#define PAMIXER \
	(char*[]) { "/usr/bin/pamixer", "--get-mute", "--get-volume", NULL }

#define PAVUCONTROL \
	(char*[]) { "/usr/bin/pavucontrol-qt", NULL }
#define SETDEFAULTVOL \
	(char*[]) { "/usr/bin/pamixer", "--set-volume", "50", NULL }
#define TOGGLEMUTE \
	(char*[]) { "/usr/bin/pamixer", "--toggle-mute", NULL }

void volumeu(char* str, int sigval) {
	char buf[BUFLENGTH];
